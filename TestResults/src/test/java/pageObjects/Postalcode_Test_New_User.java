...
public class Postalcode_Test_New_User extends ReusableMethods {
  ...
  public Postalcode_Test_New_User registerNewUserForPostalCodeTest(HashMap<String,String> NewRegistration) throws Exception {
	...
	// Enter all registration details until postal code
	    ...
	    waitTime();
		text(email, data.emailForPostalCodeTest, "typing email id name");
		waitTime();
	    ...
	    text(Display_name,data.phoneForPostalCodeTest,"Typing the display name");
...
//		waitTime();
 ...
 public Postalcode_Test_New_User entervalidPostalCodeAndVerifyTextboxes() throws Exception {
   ...
   public String getPostalCode()  throws Exception {
     String Postalcoce = "//input[@id='postalCode']";
	scrollToView(Postalcoce,"Postalcoce");
   ...
   public Postalcode_Test_New_User	modifyPostalCodeSaveAndVerifyChanges() throws Exception {
	   ...
	   Testdata data = new Testdata();
		...
		waitTime();
		//text(addressLine1,data.addressLine1, "Setting address into address line 1 filed");
		waitTime();
	...
	return this;
            ...
            // Modify postal code to xx500x
		 /*	EncashUIMethods.setPostalCode(data.xx500x);
			// Verify address and town box is displayed
			assertTrue(EncashUIMethods.isAddressBoxDisplayed(), "Address Line1 textbox is not displayed");
			assertTrue(EncashUIMethods.isTowncityTextBoxDisplayed(), "Town/City textbox is not displayed");
			// Enter details into address
			EncashUIMethods.setCustomAddress(data.addressLine1);
			EncashUIMethods.setCustomTown(data.town_city);
			// EncashUIMethods.giveConsent();
			// Save changes
			EncashUIMethods.clickContinueButton();
			EncashUIMethods.skipemailverify();
			// Verify new postal code is saved
			EncashUIMethods.clickEditProfile();
			EncashUIMethods.setEnterVerifyPasswordText(data.password);
			EncashUIMethods.clickVerifyButton();
			assertEquals(EncashUIMethods.getPostalCode(), data.xx500x, "Postal code verification failed.");
			assertEquals(EncashUIMethods.getAddressText().trim(), data.addressLine1.concat(", " + data.town_city),
